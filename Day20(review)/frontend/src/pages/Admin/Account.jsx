...
import { getAdmin, setAdmin } from "../../slices/adminSlice"
...
const Account = () => {
    ...
    const [banner, setBanner] = useState(admin?.banner || "");
    ...
    useEffect(() => {
        const authentication = async () => {
            await AdminService.getAdminData(localStorage.getItem('id')).then((res) => {
    ...
    const handleChange = async (url) => {
        if (type === "banner") {
            await AdminService.updateBanner({ id: localStorage.getItem('id'), file: url }).then(async (response) => {
                if (response.status == 200) {
                    await AdminService.getVendorData(localStorage.getItem('id')).then((res) => {
            ...
            await AdminService.updateProfile({ id: localStorage.getItem('id'), file: url }).then(async (response) => {
                if (response.status == 200) {
                    await AdminService.getAdminData(localStorage.getItem('id')).then((res) => {
                        dispatch(setAdmin(res.data));
    ...
    return (
        <div className='mt-20 flex flex-col min-h-screen bg-lightGray2'>
            <Sidebar />
            <Toaster />
            {showUploader && <FileUploader fileRef={fileRef} setImageUrl={setFileUrl} setShowUploader={setShowUploader} handleChange={handleChange} />}
            <div className="ml-56 mt-5  flex flex-col">
                <div className="flex relative justify-between h-96 w-full object-cover rounded-xl bg-white shadow-xl group">
                    {
                        loading ?
                            ...
                            :
                            <img src={banner} className="max-h-96 w-full object-cover rounded-xl shadow-xl" />
                    }
                    {/* <input ref={fileRef} accept="image/*" multiple={false} onChange={handleChange} type="file" name="" style={{ display: "none" }} id="" /> */}
                    <div onClick={() => { setShowUploader(true); setType('banner') }} className="absolute text-white bg-opacity-50 cursor-pointer hover:text-blue right-4 bottom-0 flex justify-center items-center">
                    ...
                    <div className="absolute -bottom-20 right-[40%] h-60 w-60 rounded-full">
                        <img src={profile} className="h-60 object-cover rounded-full shadow-xl" />
                        {/* <input ref={fileRefProfile} accept="image/*" multiple={false} onChange={handleChangeProfile} type="file" name="" style={{ display: "none" }} id="" /> */}
                        <div onClick={() => { setShowUploader(true); setType('profile') }} className="absolute text-white bg-opacity-50 cursor-pointer hover:text-blue right-7 bottom-10 flex justify-center items-center">
                ...
                <div className="flex justify-between items-start mt-14">
                    <div className="flex-1 bg-white rounded-lg w-5/12 shadow-xl p-8">
                        ...
                        <ul className="mt-2 text-gray-700">
                            ...
                            <li className="flex items-center border-b py-2 space-x-2">
                                ...
                                <a href="#" title="Facebook">
                                    <svg className="w-5 h-5" id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 506.86 506.86"><defs></defs><path className="cls-1" fill='#1877f2' d="M506.86,253.43C506.86,113.46,393.39,0,253.43,0S0,113.46,0,253.43C0,379.92,92.68,484.77,213.83,503.78V326.69H149.48V253.43h64.35V197.6c0-63.52,37.84-98.6,95.72-98.6,27.73,0,56.73,5,56.73,5v62.36H334.33c-31.49,0-41.3,19.54-41.3,39.58v47.54h70.28l-11.23,73.26H293V503.78C414.18,484.77,506.86,379.92,506.86,253.43Z"></path><path fill='#fff' className="cls-2" d="M352.08,326.69l11.23-73.26H293V205.89c0-20,9.81-39.58,41.3-39.58h31.95V104s-29-5-56.73-5c-57.88,0-95.72,35.08-95.72,98.6v55.83H149.48v73.26h64.35V503.78a256.11,256.11,0,0,0,79.2,0V326.69Z"></path></svg>
                                </a>
